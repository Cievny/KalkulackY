...
//
// Pravidlo: pod /tools/ a /cz/tools/ je VŠETKO chránené (default-deny) vrátane
// samostatných kalkulačiek – verejné sú len login stránky, kioskové brány
// (prihlasujú sa kódom priamo v stránke), adventný kalendár odkazovaný
// z verejného webu a JS, ktoré tieto verejné stránky potrebujú.
...
const PUBLIC_DIRS = [
  ...
  '/tools/adventny-kalendar/'                          // odkazovaný z verejného webu (prakticke-nastroje.html)
];
...
export function normalize(pathname) {
  ...
  // bodkové segmenty a spätné lomky po dekódovaní (/verejna/%2e%2e/evk/) nechávame
  // chránené – nespoliehame sa na to, ako ich znormalizuje súborový server
  if (p.indexOf('\\') >= 0 || /(^|\/)\.\.?(\/|$)/.test(p)) return null;
