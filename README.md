...
`middleware.js` (logika v `lib/brana.mjs`, testy v `tests/brana.mjs`) beží na Verceli **pred podaním súboru**. Pod `/tools/` a `/cz/tools/` je všetko chránené (default-deny) **vrátane samostatných kalkulačiek** (ALI, AorticTrauma, CAR, CEAP, SVP, Tromboflebitída, Villa, WELLS, claudication, defektologia – tie majú navyše klientskú `checkAuth()`). Výnimky vymenované v `lib/brana.mjs`: login stránky, kioskové brány `/tools/tv/` a `/tools/velin/` (prihlasujú sa kódom priamo v stránke), adventný kalendár odkazovaný z verejného webu a `auth.js`. **Nový nástroj je chránený automaticky** – ak má byť verejný, treba ho pridať do `PUBLIC_DIRS`.
